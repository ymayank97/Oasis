...
export default function HomePage() {
  ...
  const handleSubmitQuery = () => {
    ...
    setPosts((old) => [
      ...old,
      PostObject(statement, 0, 0, Constants.interactions.none, [], "me", null),
    ]);
    setStatement("");
  };
    ...
    <div className="flex justify-center items-center min-h-screen">
      <div className="flex flex-col items-center gap-2">
        ...
        {status !== "notConnected" ? null : (
          <button
            onClick={connect}
            className="px-4 py-2 rounded-xl mt-2 text-white bg-purple-700 font-semibold"
          >
            Connect to MetaMask
          </button>
        )}
      ...
      <div className="flex flex-col max-w-xl w-full gap-4">
        {posts.map((post, idx) => (
          <Post
            ...
            removePostCallback={() =>
              setPosts((posts) => posts.filter((_, postIdx) => postIdx !== idx))
            }
                  ...
                  postIdx !== idx
                    ...
                    : {
                        ...
                        comments: post.comments.filter(
                          (_, commentIdx) => commentIdx !== commentId
                        ),
                      }
                )
              )
